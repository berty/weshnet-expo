...
// Unary generator
const createUnaryMethod = <M extends pbjs.Method>(
  method: M,
  unaryCall: unknown,
) => {
...
const createUnaryMethodList = <S extends pbjs.Service>(
  ...
  const methods = {}
  Object.keys(service.methods || {}).forEach(
    (key: keyof typeof service.methods) => {
...
// Streams generator
const createStreamMethod = <M extends pbjs.Method>(
  method: M,
  streamCall: unknown,
) => {
  const requestType = method.resolvedRequestType
...
export const createServiceClient = <
  T extends typeof pbjs.rpc.Service,
